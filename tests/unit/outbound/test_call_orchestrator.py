...
class TestCallOrchestrator:
    async def test_build_props_creates_cartesian_product(self, orchestrator, mocker):
    ...
    async def test_run_executes_correct_number_of_rounds(self, orchestrator, mocker):
    ...
    async def test_run_round_submits_tasks_to_concurrent_runner(self, orchestrator, mocker):
        mock_props = MagicMock()
        ...
        mocker.patch("livekit_voice_call_runner.outbound.call_orchestrator.CallRunner")
