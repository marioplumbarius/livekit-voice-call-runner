...
class TestInboundWorker:
    async def test_entrypoint_calls_ctx_connect_first(self, mocker):
    ...
    async def test_entrypoint_calls_shutdown_in_finally(self, mocker):
    ...
    def test_run_worker_sets_dev_argv(self, mocker):
        """run_worker must inject 'dev' into sys.argv before calling agents_cli."""
        mock_run_app = mocker.patch("livekit.agents.cli.run_app")

        from livekit_voice_call_runner.inbound.worker import run_worker

        original_argv = sys.argv.copy()
        run_worker(instructions="Test instructions")

        mock_run_app.assert_called_once()
        # sys.argv should have been set to [argv[0], "dev"]
        assert sys.argv[1] == "dev"

        sys.argv = original_argv

    async def test_entrypoint_passes_rtc_room_to_listen_to_room(self, mocker):
