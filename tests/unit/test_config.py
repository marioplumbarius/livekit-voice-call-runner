...
def _reload_config(module_path: str):
    """Reload a config module and reset its singleton so env changes take effect."""
    import importlib
    import sys
...
class TestSharedConfigLazyLoading:
    ...
    def test_get_config_is_cached(self, shared_env):
        module = _reload_config("livekit_voice_call_runner.config.shared")
...
class TestOutboundConfigLazyLoading:
    def test_importing_module_does_not_raise_without_env_vars(self):
    ...
    def test_get_config_returns_correct_values(self, outbound_env):
        module = _reload_config("livekit_voice_call_runner.config.outbound")
        cfg = module.get_config()
    ...
    def test_outbound_config_does_not_require_shared_vars(self, monkeypatch, outbound_env):
